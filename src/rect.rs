...
where T: Copy + Clone + Zero + PartialOrd + PartialEq + Add<T, Output=T> + Sub<T, Output=T> {
    ...
    #[inline]
    pub fn translate(&self, other: &TypedVector2D<T, U>) -> TypedRect<T, U> {
        TypedRect::new(
...
mod tests {
    ...
    fn test_translate() {
        ...
        let pp = p.translate(&Vector2D::new(10,15));
        ...
        let rr = r.translate(&Vector2D::new(0,-10));
    ...
    fn test_contains() {
        ...
        assert!(!r.contains_rect(&r.translate(&Vector2D::new( 0.1,  0.0))));
        assert!(!r.contains_rect(&r.translate(&Vector2D::new(-0.1,  0.0))));
        assert!(!r.contains_rect(&r.translate(&Vector2D::new( 0.0,  0.1))));
        assert!(!r.contains_rect(&r.translate(&Vector2D::new( 0.0, -0.1))));
