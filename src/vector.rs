...
where T: Copy + Mul<T, Output=T> + Add<T, Output=T> + Sub<T, Output=T> {
    ...
    /// Square of the vector's length (faster to compute than the length).
    #[inline]
    pub fn square_norm(&self) -> T { self.x * self.x + self.y * self.y }

    #[inline]
    pub fn square_norm_typed(&self) -> Length<T, U> { Length::new(self.square_norm()) }
}
...
impl<T: Mul<T, Output=T> +
        ...
        Div<T, Output=T> +
        Sqrt +
        Copy, U> TypedVector2D<T, U> {

    /// The vector's length.
    pub fn norm(&self) -> T { self.square_norm().square_root() }

    pub fn norm_typed(&self) -> Length<T, U> { Length::new(self.norm()) }

    /// Returns the normalized version of this vector (norm equal to 1).
    pub fn normalized(&self) -> TypedVector2D<T, U> {
        let n = self.norm();
        return TypedVector2D::new(self.x / n, self.y / n);
    }
}
...
impl<T: Copy + AddAssign, U> AddAssign<TypedVector2D<T, U>> for TypedVector2D<T, U> {
...
impl<T: Copy + SubAssign, U> SubAssign<TypedVector2D<T, U>> for TypedVector2D<T, U> {
...
impl<T: Copy + MulAssign, U> MulAssign<T> for TypedVector2D<T, U> {
...
impl<T: Copy + DivAssign, U> DivAssign<T> for TypedVector2D<T, U> {
...
impl<T0: NumCast + Copy, U> TypedVector2D<T0, U> {
    ...
    pub fn cast<T1: NumCast + Copy>(&self) -> Option<TypedVector2D<T1, U>> {
...
impl<T: Copy + Zero, U> TypedVector3D<T, U> {
    /// Returns a 4d array with zero as the homogeneous coordinate.
    #[inline]
    pub fn to_array4d(&self) -> [T; 4] { [self.x, self.y, self.z, Zero::zero()] }
}
...
impl<T: Mul<T, Output=T> +
    ...
    // Square of the vector's length (faster to compute than the length).
    #[inline]
    pub fn square_norm(&self) -> T { self.x * self.x + self.y * self.y + self.z * self.z }
...
impl<T: Mul<T, Output=T> +
    ...
    /// Returns the normalized version of this vector (norm equal to 1).
    pub fn normalized(&self) -> TypedVector3D<T, U> {
        let n = self.norm();
        return TypedVector3D::new(self.x / n, self.y / n, self.z / n);
    }
}
...
impl<T: Copy + AddAssign, U> AddAssign<TypedVector3D<T, U>> for TypedVector3D<T, U> {
...
impl<T: Copy + SubAssign, U> SubAssign<TypedVector3D<T, U>> for TypedVector3D<T, U> {
...
impl<T: Copy + MulAssign, U> MulAssign<T> for TypedVector3D<T, U> {
    fn mul_assign(&mut self, other: T) {
        self.x *= other;
        self.y *= other;
        self.z *= other;
...
impl<T: Copy + DivAssign, U> DivAssign<T> for TypedVector3D<T, U> {
    fn div_assign(&mut self, other: T) {
        self.x /= other;
        self.y /= other;
        self.z /= other;
...
impl<T0: NumCast + Copy, U> TypedVector3D<T0, U> {
    /// Cast from one numeric representation to another, preserving the units.
    ///
    /// When casting from floating point to integer coordinates, the decimals are truncated
    /// as one would expect from a simple cast, but this behavior does not always marke sense
    /// geometrically. Consider using round(), ceil or floor() before casting.
    pub fn cast<T1: NumCast + Copy>(&self) -> Option<TypedVector3D<T1, U>> {
